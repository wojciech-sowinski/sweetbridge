	...
	function swiftbridge_content_nav($nav_id)
	{
		...
		if ($wp_query->max_num_pages > 1) {
			...
			<div id="<?php echo esc_attr($nav_id); ?>" class="d-flex mb-4 justify-content-between">
				<div>
					<?php next_posts_link('<span aria-hidden="true">&larr;</span> ' . esc_html__('Older posts', 'swiftbridge')); ?>
				</div>
				<div>
					<?php previous_posts_link(esc_html__('Newer posts', 'swiftbridge') . ' <span aria-hidden="true">&rarr;</span>'); ?>
				</div>
			</div><!-- /.d-flex -->
			<?php
		} else {
...
function swiftbridge_scripts_loader()
{
	...
	wp_enqueue_style('splideStyle', 'https://cdn.jsdelivr.net/npm/@splidejs/splide@4.1.4/dist/css/splide.min.css', array(), $theme_version, 'all'); // main.scss: Compiled Framework source + custom styles.
	wp_enqueue_style('AosCss', 'https://unpkg.com/aos@2.3.1/dist/aos.css', array(), $theme_version, 'all');
	...
	wp_enqueue_script('splideJs', 'https://cdn.jsdelivr.net/npm/@splidejs/splide@4.1.4/dist/js/splide.min.js', array(), $theme_version, true);
...
function textToBlur($text)
{
	$explode = explode(" ", $text);
	$time = 0;
	foreach ($explode as $key=>$word) {
		$time += 0.1;
		echo "<span class='blur' style='animation-delay:".$time."s;'>$word</span> ";
	}
}
function textToBlend($text)
{
	$explode = explode(" ", $text);
	$time = 0;
	foreach ($explode as $key=>$word) {
